...
trait Eval extends LMS_Base {
  type Cont = Int => Int
  type Env = List[scala.collection.mutable.Map[String,Int]]

  type Fun = Int => Int
  var funs: scala.collection.mutable.Map[String,Fun] = scala.collection.mutable.Map()
  def apply_fun(n: String, args: List[Int]): Int = funs(n)(args(0)) // limited to only one parameter

  def base_eval(exp: Term, env: Env, cont: Cont): Int = {
    exp match {
      case I(a) => cont(a)
  ...
  def eval_var(exp: Term, env: Env, cont: Cont): Int = exp match {
    case S(x) => get(env, x) match { case Some(v) => cont(v) }
  }
  def eval_if(exp: Term, env: Env, cont: Cont): Int = exp match {
    case P(_, P(c, P(a, P(b, N)))) => base_eval(c, env, { cv =>
      if (cv == 0) base_eval(b, env, cont) else base_eval(a, env, cont)
    })
  }
  def eval_set_bang(exp: Term, env: Env, cont: Cont): Int = exp match {
    case P(_, P(S(x), P(rhs, N))) => base_eval(rhs, env, { v =>
      set(env, x, v) match { case Some(v) => cont(v) }
    })
  }
  def eval_begin(exp: Term, env: Env, cont: Cont): Int = exp match {
    case P(e, N) => base_eval(e, env, cont)
    case P(e, es) => base_eval(e, env, { _ => eval_begin(es, env, cont) })
  }
  def eval_define(exp: Term, env: Env, cont: Cont): Int = exp match {
    ...
    case P(_, P(P(r@S(name), params), body)) => {
      ...
      val f = {args: List[Int] =>
        eval_begin(body, extend(env, ps, args.map{v => v}), { v => v })}
      val v = ps.length match {
          case 1 => {x: Int => f(List(x))}
      }
      funs(name) = v
      cont(0)
    }
  }
  def eval_application(exp: Term, env: Env, cont: Cont): Int = exp match {
  ...
  def evlist(exp: Term, env: Env, k: List[Int] => Int): Int = exp match {
    case N => k(Nil)
  ...
  def extend(env: Env, params: List[String], args: List[Int]): Env =
    collection.mutable.Map(((params zip args) : _*))::env

  def get(env: Env, x: String): Option[Int] = env match {
    case Nil => None
  ...
  def set(env: Env, x: String, v: Int): Option[Int] = env match {
    case Nil => None
    case first::rest => first.get(x) match {
      case Some(_) => {
        first(x) = v
  ...
  def apply_primitive(p: String, args: List[Int]): Int =  (p, args) match {
    case ("<", List(a, b)) => if (a < b) 1 else 0
    case ("+", List(a, b)) => a+b
    case ("-", List(a, b)) => a-b
    case ("*", List(a, b)) => a*b
  ...
  def init_env: Env = (scala.collection.mutable.Map[String,Int]()) :: (Nil: Env)
}

/*
// TODO: Comment remaining of this file out once you've gotten started on trait Eval.
...
import repl._
import utils._
class ex2_lisp_Tests extends TestSuite {  before { clean() }
